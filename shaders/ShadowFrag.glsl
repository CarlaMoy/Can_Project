...
//        colour += ADS(i, FragmentPosition, FragmentNormal);
  ...
  //  FragColour = vec4(colour, 1.0) * texture(labelMap, FragmentTexCoord);
    ...
    float shadeFactor=textureProj(ShadowMap,ShadowCoord).x;
    vec4 Shadow = vec4(shadeFactor * Colour.rgb , Colour.a);


    FragColour =  texture(labelMap, FragmentTexCoord) * vec4(lightColour,1.0) * (colour) * Shadow;// * shadeFactor;
...
/*void main ()
{
 ...
 // outColour += vec4(0.1, 0.4,0.1, 1.0);
