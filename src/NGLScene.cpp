...
constexpr auto CanShader = "CanShader";
...
void NGLScene::initializeGL()
{
  ...
  shader->createShaderProgram(CanShader);
  // now we are going to create empty shaders for Frag and Vert
  ...
  // add them to the program
  shader->attachShaderToProgram(CanShader,"ShadowVertex");
  shader->attachShaderToProgram(CanShader,"ShadowFragment");
  // now we have associated this data we can link the shader
  shader->linkProgramObject(CanShader);
